...
function Invoke-Backup-And-Replace {
    ...
    Write-Host $release_backup_path  
    Write-Host $release_backup_zip  
...
function Get-Release-Asset {
    param([string]$download_path, [string]$gitlab_token, [string]$org, [string]$repo, [string]$tag, [string]$project_id)
    If (!(Test-path $download_path)) {
        New-Item -ItemType Directory -Force -Path $download_path
    }
    $download_url = "$global:url/projects/$project_id/packages/generic/$repo/$tag/$repo-$tag.zip"
    ...
    Invoke-WebRequest -Headers $headers $download_url -OutFile $zip_file
    return $zip_file
}
...
$zip_file = Get-Release-Asset $download_path $gitlab_token $org $repo $tag $project_id
